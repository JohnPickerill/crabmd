...
#TODO move these 2 functions elsewhere - this module should not be dependant on flask or our data model
def htmlArticleLink(articleId,text):
...
#TODO consider whether to do this serverside rather than using jQuery
...
class CcMarkdown(Markdown):
    ...
    def addElink(url):
        elinks.append(url) # TODO need to import the extlink class and do this properly
...
class CcRenderer(Renderer):
    ...
    def wiki_link(self, text, link):
        ...
        if matchObj is None: 
            ...
            if m is None:
                url = htmlArticleLink(link.strip(),text)
            else:
                url = htmlSnippet(m.group(1),m.group(2),text)
        else:
...
class CcInlineGrammar(InlineGrammar):  
...
class CcInlineLexer(InlineLexer):
    ...
    def __init__(self, renderer, rules=None, **kwargs):
        ...
        if rules is None:
            ...
            rules = CcInlineGrammar()

					
        super(CcInlineLexer, self).__init__(renderer, rules, **kwargs)
...
class CcBlockGrammar(BlockGrammar):    
    #hotdrop = re.compile(r'^(?:^|\n)(?:{hotdrop:)([^}\n]*)[^\n\S]*\n([^}\n]*)([^}]*)\n}')   
    blk  = re.compile(r'(?:^|\n){{blk!([a-zA-Z0-9-_]+):\s*\n([\s\S]*?)\n(?:blk!\1)}}[^\n]*(?:\n|$)')
    box  = re.compile(r'(?:^|\n){{(box(?:![a-z]+)?):[ \t]*([a-zA-Z0-9 \t]*)\n([\s\S]*?)\n(?:\1)}}[^\n]*(?:\n|$)')
    drop = re.compile(r'(?:^|\n){{(drop\!?(?:(?<=\!)([_a-z0-9]+))?):[ \t]*([a-zA-Z0-9 \t]*)\n([^\n]*)\n([\s\S]*?)\n\1}}[^\n]*(?:\n|$)')
    #item = re.compile(r'(?:^|\n){{item:[ \t]*([\S]+)\s*(\S*)?\s*}}[^\n]*(?:\n|$)')
    
class CcBlockLexer(BlockLexer):
    default_rules = copy.copy(BlockLexer.default_rules)
    #default_rules.insert(1,'hotdrop')
    ...
    def __init__(self,  rules=None, **kwargs):
        if rules is None:
            # use the inline grammar
            rules = CcBlockGrammar()

        super(CcBlockLexer, self).__init__(rules, **kwargs)

    def parse_blk(self, m):
        #TODO check why do this
        src = m.group(0)
        #raise
